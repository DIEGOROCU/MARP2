...
\begin{frame}[fragile]{Definición de los nodos}
    Definamos los nodos que usaremos para representar el estado actual en el árbol de decisiones:
    \begin{lstlisting}[language=c++, basicstyle=\ttfamily\scriptsize]
struct Nodo {
    vector<int> asignacion; // Vector que representa la asignación de productos a supermercados
    int costeActual; // Coste acumulado hasta el nodo actual
    int cotaOptimista; // Cota optimista para el nodo actual, la prioridad
    int productoActual; // Índice del producto que se va a asignar en este nodo
};
    \end{lstlisting}
\end{frame}
...
int ramificacionYPoda() {
    priority_queue<Nodo> pq;
    Nodo raiz; // Inicializamos
    int mejor = raiz.cotaPesimista; // estimacion inicial con cota pesimista
    pq.push(raiz);
    while (!pq.empty()) {
        Nodo nodo = pq.top(); pq.pop();
        if (nodo.cotaOptimista >= mejor) continue; // poda
        if (nodo.productoActual == n) { // hoja: solucion completa
            mejor = min(mejor, nodo.costeActual); continue;
        }
        for (int j = 0; j < m; j++) {
            if (nodo.asignacion[j] >= 3) continue; // restriccion capacidad
            Nodo hijo;
            hijo.asignacion = nodo.asignacion; hijo.asignacion[j]++;
            hijo.costeActual = nodo.costeActual + precios[nodo.productoActual][j];
            hijo.productoActual = nodo.productoActual + 1;
            hijo.cotaOptimista = hijo.costeActual +
                cotaOptimista(precios, hijo.asignacion, hijo.productoActual);
            if (hijo.cotaOptimista < mejor) {
                mejor = min(mejor, hijo.cotaPesimista);
                pq.push(hijo); // Segun la prioridad de la cota optimista
            }
        }
    }
    return mejor;
